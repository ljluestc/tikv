...
fn test_cache_with_selection() {
    ...
    // Build a select request with a WHERE clause (selection executor).
    // The condition `count > 2` filters rows where the `count` column > 2.
    let col_cnt = product.col("count");
    let req = DagSelect::from(&product)
        .where_expr(ExprDefBuilder::column_ref(2, FieldTypeTp::LongLong).gt(ExprDefBuilder::constant_int(2)))
        .build_with(ctx, &[0]);
